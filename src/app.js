...
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
  ...
  // Only proceed if all required DOM elements exist
  if (buyTab && sellTab && orderbookDiv) {
    let orderData = {};
  ...
  function addNewOrder(orderType) {
    ...
    // Post to API
    postOrder(orderType, { quantity: newOrder[0], price: newOrder[1] }).then(result => {
      if (result.success) {
...
export { generateRandomOrder, postOrder };
